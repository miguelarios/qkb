...
describe("ingest/pipeline (structural)", () => {
  ...
  it("de-indexes a note that loses its id and a genuine file deletion (#33)", async () => {
    writeNote(vault, "a.md", ID1);
    writeNote(vault, "b.md", ID2);
    await ingestVault(conn, cfg);

    // opt out: the only requirement is an id, so dropping it de-indexes
  ...
  it("applies a frontmatter-only change without a full re-index", async () => {
    ...
    // same body; a new title and a changed ordinary property
    writeNote(vault, "a.md", ID1, { context: "homelab-updated", extra: "title: Renamed\n" });
    const stats = await ingestVault(conn, cfg);

    expect(stats.unchanged).toBe(1);
    expect(stats.updated).toBe(0); // not the body-changed path
    const title = (
      conn.prepare("SELECT title FROM documents WHERE id = ?").get(ID1) as { title: string }
    ).title;
    expect(title).toBe("Renamed");
    const ftsTitle = (
      conn.prepare("SELECT title FROM documents_fts WHERE doc_id = ?").get(ID1) as {
        title: string;
      }
    ).title;
    expect(ftsTitle).toBe("Renamed");
    const ctx = conn
      .prepare("SELECT value FROM metadata WHERE document_id = ? AND key = 'context'")
      .get(ID1) as { value: string };
    expect(ctx.value).toBe("homelab-updated");
  });
  ...
  it("a note whose date stops parsing stays indexed (file mtime fallback); losing its id de-indexes it", async () => {
    writeNote(vault, "a.md", ID1);
    writeNote(vault, "b.md", ID2);
    await ingestVault(conn, cfg);

    // a.md: its only date is a Templater placeholder that doesn't parse
    writeFileSync(
      join(vault, "a.md"),
      `---\nid: ${ID1}\ncreated: <% tp.date.now() %>\n---\n\nstill here\n`,
    );
  ...
  it("a brand-new note with an unparseable date is indexed with its file mtime (#33)", async () => {
    writeFileSync(
      join(vault, "undated.md"),
      `---\nid: ${ID1}\ndate: <% tp.date.now() %>\n---\n\nx\n`,
    );
    const stats = await ingestVault(conn, cfg);
    expect(stats.indexed).toBe(1);
  ...
  it("counts notes without an id, indexes undated notes, and drives progress to completion", async () => {
    writeNote(vault, "good.md", ID1);
    writeFileSync(
      join(vault, "no-id.md"),
      "---\ncontext: homelab\ncreated: 2026-01-01\n---\nbody\n",
    );
    writeFileSync(join(vault, "no-date.md"), `---\nid: ${ID2}\n---\nbody\n`);
    writeFileSync(join(vault, "plain.md"), "Just text, no frontmatter.\n");
    ...
    // Missing ids are counted, not reported as errors.
    expect(skips).toEqual([]);
    expect(stats.withoutId).toBe(2);
    expect(stats.indexed).toBe(2); // good.md + no-date.md
